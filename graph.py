...
def bar_plot(results, title):
    pylab.clf()
    pylab.figure().autofmt_xdate()
    pylab.bar(range(len(results)),results)
    pylab.title(title)
...
def plot_2d(results_list, title,legend=None, colors=None, limits=None):
    ...
    if not legend ==  None:pylab.legend(legend,'best')
    if not os.path.exists('./graphs'): os.makedirs('./graphs')
    title = 'graphs/' +title + FILETYPE
    pylab.savefig(title)
